...
namespace Cat
{
  ...
  public class BenNumber
  {
    ...
    public BenNumber (IncType type, string _strFormat, double min, double max, double current)
    {
    ...
    public void AddUIText (TextMeshProUGUI ui)
    {
      UIText = ui;
    ...
    {
      if (eType == IncType.VALUE)
        return fCurrent.ToString(strFormat);
      else if (eType == IncType.VALUEwithMAX)
        return fCurrent.ToString(strFormat) + " / " + fMax.ToString(strFormat);
