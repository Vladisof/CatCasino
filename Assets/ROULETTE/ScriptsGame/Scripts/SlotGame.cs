...
namespace Cat
{
  ...
  public class Symbol
  {
    ...
    public Symbol (string path, SymbolType _type, string _frequency, string _reward)
    {
      BaseSet(path, _type, _reward);
    ...
    public Symbol (string path, SymbolType _type, int _frequency, string _reward)
    {
      BaseSet(path, _type, _reward);
    ...
    private void BaseSet (string path, SymbolType _type, string _reward)
    {
      type = _type;
      SetPrefab((GameObject)Resources.Load(path, typeof(GameObject)));
    ...
    public void SetPrefab (GameObject go)
    {
      ...
      if (go != null)
      {
        BenObjectPool.AddPoolItem(go, 20);
      }
    }
  }
  ...
  public class GameResult
  {
    ...
    public float SpinWin;
    public List<WinItem> Wins = new List<WinItem>();
    public int NewFreeSpinCount;
    public int FreeSpinCount;
    public int FreeSpinTotalCount;
    public float FreeSpinTotalWins;
    public int FreeSpinAccumCount;
    public float FreeSpinAccumWins;
  ...
  public class SlotGame : MonoBehaviour
  {
    ...
    public float Gold
    {
      get => characterMoney.characterMoney.Value;
      set => characterMoney.characterMoney.Value = (int)value;
    }
    ...
    private Random _randomDotNet = new Random();
    private MT _randomMersenneTwister = new MT();

    public List<UIReel> Reels = new List<UIReel>();
    ...
    public bool InSplashShow;

    private int _winOffset;
    private float _winShowAge;
    private int _winShowBlinkCount = 2;
    private float _winShowPeriod = 1.2f;

    public IntEvent ReelStopCallback;
    public UnityEvent SpinEndCallback;
    public IntEvent SplashShowCallback;
    public UnityEvent SplashEndCallback;

    private BenChoice [] _choice;
    private List<int> _deck = new List<int>();
    public GameResult gameResult = new GameResult();
    public GameResult resultTest;
    ...
    public void SetReelCount (int value)
    {
      ...
      for (int x = 0; x < Reels.Count; ++x)
      {
        ...
        for (int y = 0; y < Reels[x].Symbols.Length; ++y)
        {
          ...
          {
            BenObjectPool.Unspawn(Reels[x].Symbols[y]);
          }
        }
      ...
      {
        GameObject go = new GameObject("Reel" + x);
        go.AddComponent<RectTransform>();
        go.transform.SetParent(transform);
        go.transform.localPosition = Vector3.zero;
        go.transform.localScale = Vector3.one;

        UIReel reel = go.AddComponent<UIReel>();
        reel.Init(this, x);
        Reels.Add(reel);
    ...
    {
      float fStart = -(float)(Reels.Count - 1) * (SymbolSize + MarginSize) / 2.0f;
      ...
      for (int x = 0; x < Reels.Count; ++x)
      {
        ...
        fStart += SymbolSize + MarginSize;

        float fYStart = -(float)(RowCount - 1) * (SymbolSize + MarginSize) / 2.0f;

        for (int y = 0; y < Reels[x].Symbols.Length; ++y)
        {
          Reels[x].SymbolPos[y] = new Vector3(0, fYStart, 0);
          Reels[x].Symbols[y].transform.localPosition = Reels[x].SymbolPos[y];
          fYStart += SymbolSize + MarginSize;
        }
      }
    }
    ...
    public void SymbolInsert (int ID)
    {
      ...
      Symbols.Insert(ID, new Symbol("Noname", SymbolType.Normal, 0, "0,0,0,0,0"));
    ...
    private void Start()
    {
      ...
      SymbolSizePlusMargin = SymbolSize + MarginSize;
      DampingHeight = SymbolSize * 0.4f;
    ...
    private void Update()
    {
      ...
      {
        WinItem wi = gameResult.Wins[_winOffset];
        Line ld = GetLine(wi.Line);

        float fValue = _winShowBlinkCount * 2.0f / _winShowPeriod;
        float fScale = Mathf.PingPong(_winShowAge * fValue, 1.0f) * 0.2f + 1.0f;

        imgSelection.color = ld.color;
        imgSelection.transform.localPosition = _markers[wi.Line].go.transform.localPosition;
        ...
        for (int x = 0; x < Reels.Count; ++x)
        {
          ...
          int SymbolIdx = _deck[RowCount * x + ld.Slots[x]];
          Symbol sd = GetSymbol(_deck[RowCount * x + ld.Slots[x]]);
        ...
        _winShowAge += Time.deltaTime;

        if (_winShowAge > _winShowPeriod)
        {
          _winShowAge -= _winShowPeriod;

          _winOffset++;

          if (_winOffset >= gameResult.Wins.Count)
          {
            _winOffset = 0;
      ...
      if (!_inSpin && _inResult && !InSplashShow)
      {
      ...
      if (_activeReel == -1)
      {
        ...
        if (_spinAge > 0.5f)
        {
          ...
          ApplyResult();
          Reels[_activeReel].Stop();
        }
      } else
      {

        if (Reels[_activeReel].Completed())
        {
          if (ReelStopCallback != null)
          {
            ReelStopCallback.Invoke(_activeReel);
          }
    ...
    public void CreateChoice()
    {
      ...
      _choice = new BenChoice[Reels.Count];

      for (int x = 0; x < Reels.Count; ++x)
      {

        _choice[x] = new BenChoice();
        _choice[x].Clear();
        ...
        for (int i = 0; i < Symbols.Count; ++i)
        {
          ...
          if (sd.frequency[x] != 0)
          {
            _choice[x].Add(i, sd.frequency[x]);
          }
        }
      }
    }
    ...
    public void CheckWin()
    {
      ...
      gameResult = CheckSpinWin(_deck, gameResult);
      float Gold = characterMoney.characterMoney.Value;
      ...
      {
        if (gameResult.Wins[i].Matches != Reels.Count)
        {
          continue;
        }
    ...
    public void MarkerCreate()
    {
      ...
      {
        GameObject go = Instantiate(prefMarker, Vector3.zero, Quaternion.identity);
        go.name = "Marker " + i.ToString("00");
        go.transform.SetParent(_tr);
        go.transform.localScale = Vector3.one;
        _markers.Add(new Marker(i, GetLine(i), go));
      }
      ...
      for (int i = 0; i < Lines.Count; ++i)
      {
        if (i < Half)
        {
          sortList[i].go.transform.localPosition = new Vector3(-515.0f + (i % 2 == 1 ? 26 : 0), -Quad * 22.0f + i * 22.0f, 0);
        } else
        {
          sortList[i].go.transform.localPosition = new Vector3(515.0f - (i % 2 == 0 ? 26 : 0), -Quad * 22.0f + (i - Half) * 22.0f, 0);
        }
      }
    }
    ...
    public void LineSet (int _Line)
    {
      ...
      TotalBet = Line * RealBet;

      for (int i = 0; i < Lines.Count; ++i)
      {
        _markers[i].image.color = i < Line ? GetLine(i).color : new Color(0.5f, 0.5f, 0.5f, 1.0f);
        _markers[i].text.color = i < Line ? Color.white : Color.gray;
      }
    }
    ...
    {
      return Gold >= TotalBet && !_inSpin && !_inResult ? true : false;
    }
    ...
    public List<int> GetDeck()
    {
      ...
      for (int x = 0; x < Reels.Count; ++x)
      {
        for (int y = 0; y < RowCount; ++y)
        {
          ...
          {
            RandomValue = UnityEngine.Random.Range(0, _choice[x].ValueTotal);
          } else if (rngType == RNGType.DotNetRandom)
          {
            RandomValue = _randomDotNet.Next(0, _choice[x].ValueTotal);
          } else if (rngType == RNGType.MersenneTwister)
          {
            RandomValue = _randomMersenneTwister.RandomRange(0, _choice[x].ValueTotal - 1);
          }

          //get selected symbol and add to result list
          int SymbolIdx = _choice[x].Choice(RandomValue);
    ...
    public void ApplyResult()
    {
      ...
      // Bonus, Scatter Spin Effect
      {
        ...
        for (int x = 0; x < Reels.Count; ++x)
        {
          ...
          for (int y = 0; y < RowCount; ++y)
          {
            SymbolType eType = GetSymbol(_deck[RowCount * x + y]).type;

            if (eType == SymbolType.Scatter)
            {
              iScatterSum++;
            }

            if (eType == SymbolType.Bonus)
            {
              iBonusSum++;
            }
          }
    ...
    public GameResult CheckSpinWin (List<int> iDeck, GameResult result)
    {
      ...
      for (int i = 0; i < result.Line; ++i)
      {
        ...
        if (sd.type == SymbolType.Normal && matchCount != 0 && sd.reward[matchCount - 1] != 0)
        {
          ...
          float LineWin = sd.reward[matchCount - 1] * result.RealBet;
